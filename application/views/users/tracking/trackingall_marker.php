        ...
        <div class="row">
            <div class="panel panel-flat">
                <div class="panel-body">
                    <legend class="text-bold"><i class="fa fa-street-view"></i> Tracking 
                    </legend>
                                ...
                                <?php foreach ($userLocations as $location) : ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="loc-update-time">
                                            ...
                                            $tanggal = date('d-m-Y', $timestamp); // Format tanggal (misalnya: 04-10-2023)
                                            $jam = date('H:i:s', $timestamp); // Format jam (misalnya: 14:30:00)

                                            // Gunakan CSS inline untuk mengubah warna teks jam menjadi hijau
                                            echo $jam . ' <span style="color: green; font-weight: bold;"> - ' . $tanggal . '</span>';
                                            ?>
                                        </td>
...
<script>
    ...
    function createMarker(latitude, longitude, locationInfo, username, locUpdateTime) {
        var marker = L.marker([latitude, longitude]).addTo(userMap);
        ...
        }).setContent(`<b>${username}</b><br>${locationInfo}<br>${locUpdateTime}`);
        marker.bindTooltip(tooltip).openTooltip();

        // Tambahkan juga popup yang dapat ditutup
        var popup = L.popup().setContent(`<b>${username}</b><br>${locationInfo}<br>${locUpdateTime}`);
    ...
    function updateMapWithMarkers() {
        $(".lokasi-data").each(function() {
            ...
            var locUpdateTime = $(this).closest("tr").find(".loc-update-time").text();

            // Create a marker with username and location info in the popup
            createMarker(latitude, longitude, locationInfo, username, locUpdateTime);
        });
    }

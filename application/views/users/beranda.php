...
<body>
    <!-- <header>
        <div class="nav">
            ...
            <div class="promo" style="background: transparent;">
                <i class="fa fa-bell gradient-icon round-icon" style="font-size: 20px; cursor: pointer; position: relative; color:#fff" onclick="animateBell(); window.location.href='users/memo'">
                </i>
            </div>
        </div>
    </header> -->
            ...
            <div class="text">
                ...
                <div class="headerText">
                    <h4>
                        ...
                        <img src="<?php echo $thumbnailURL; ?>" alt="" style="width: 30px; height: 30px; border-radius: 50%; border: 2px solid white; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5); object-fit: cover;" loading="lazy">
                        </a>
                        <span style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5); font-weight: 600; font-size: 14px"><?php echo ucwords($nama); ?></span>
                        <br>
                        <small style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5); font-size: 14px">
                            <?php echo ucwords($levelText); ?>
                            ( Kantor
                            <?php echo $kodeKantorText = getKodeKantorText($cek->kode_kantor); ?> )
                        </small>
    ...
    <!-- notifikasi ulang tahun user -->
    <!-- <?php if (!empty($cek->tgl_lahir) && date('m-d', strtotime($cek->tgl_lahir)) == date('m-d')) : ?>
        <div class="notifikasi" id="notification">
            <div class="feature" id="swipeContainer">
                <div class="choose">
                    <ul class="notifikasi-content">
                        <li>
                            <i class="fa fa-birthday-cake"></i>&nbsp Selamat ulang tahun!!! <br>
    ...
    <?php endif; ?> -->
    <!-- end notifikasi ulang tahun user -->
    ...
    <!-- menu custom user -->
    <!-- <section class="recomend" style="margin-left: 1rem; margin-right: 1rem; border-radius: 10px; padding-top: 10px; padding-bottom: 0px; border: 0.1px solid orange;">
        <div class="recomendWrapper swiper mySwiper">
            <div class="cardWrapper swiper-wrapper">
                <?php if (!empty($cek->custom_menu)) : ?>
                    <?php $customMenuItems = explode('</li>', $cek->custom_menu); ?>
                    <?php foreach ($customMenuItems as $menuItem) : ?>
                        <?php if (!empty($menuItem)) : ?>
                            <div class="card swiper-slide">
                                <ul class="custom-menu" style="list-style: none;" id="customMenu">
    ...
    <!-- Pin Informasi -->
    <!-- <?php
            if (!empty($memo) && $memo->num_rows() > 0) {
                $have_pin = false;
                foreach ($memo->result() as $info_pin) {
                    if ($info_pin->status_pin == 1) {
                        $have_pin = true;
                        break;
                    }
                }
                if ($have_pin) {
            ?>
            <section class="menuCustom" style="margin: 1rem;">
                <div class="tipsContent">
                    <div class="tipsWrapper swiper mySwiper">
                        <div class="cardWrapper swiper-wrapper-pin">
                            <?php foreach ($memo->result() as $info_pin) :
                                if ($info_pin->status_pin == 1) :
                            ?>
                                    <div class="card swiper-slide">
                                        <a href="<?php echo site_url('users/memo/lihat/' . $info_pin->id_memo); ?>" style="color: #333;">
                                            <i class="fa fa-bell" aria-hidden="true"></i>&nbsp<small><?php echo $info_pin->judul_memo; ?></small>
                                        </a>
                                    </div>
                            <?php
                                endif;
    ...
    <!-- <section class="feature" style="margin-top: 8px;">
        <?php if (!empty($menu_category)) : ?>
            <div class="choose">
                <div class="search-bar active"> 
                    <input type="text" id="searchMenu" placeholder="Cari Menu..." class="search-input" onkeyup="searchMenu()" />
                </div>
            </div>

            <ul style="display: contents;">
                <div class="menuCategori" id="menuList">
                    <?php foreach ($menu_category as $menu) : ?>
                        <li class="menu-item">
                            <div class="card">
                                <a class="icon" href="<?php echo base_url($menu->url); ?>" data-menu-name="<?php echo $menu->menu_name; ?>">
                                ...
                                <p class="menu-name"><?php echo $menu->menu_name; ?></p>
                            </div>
                        </li>
                    <?php endforeach; ?>
                </div>
                ...
                <?php if ($imsakiyah) : ?>
                    <div class="imsakiyah-schedule">
                        <div class="schedule-item">
                            <span>🌙 Imsak</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->imsak)); ?></span>
                        </div>
                        <div class="schedule-item">
                            <span>🌅 Subuh</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->subuh)); ?></span>
                        </div>
                        <div class="schedule-item">
                            <span>☀️ Zuhur</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->zuhur)); ?></span>
                        </div>
                        <div class="schedule-item">
                            <span>🌇 Ashar</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->ashar)); ?></span>
                        </div>
                        <div class="schedule-item">
                            <span>🌆 Maghrib</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->maghrib)); ?></span>
                        </div>
                        <div class="schedule-item">
                            <span>🌌 Isya</span>
                            <span><?php echo date('H:i', strtotime($imsakiyah->isya)); ?></span>
                        </div>
                    </div>
                <?php else : ?>

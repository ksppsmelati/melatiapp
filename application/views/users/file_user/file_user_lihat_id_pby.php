...
<script async>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadingTask = pdfjsLib.getDocument(fileUrl); // Load the PDF file using PDF.js
    ...
    loadingTask.promise.then(function(pdf) {
      ...
      // Get the initial scale based on container size
      pdf.getPage(1).then(function(page) {
        const viewport = page.getViewport({
          scale: 1
        });
        currentScale = fileViewer.offsetWidth / viewport.width; // Set scale based on container width
        renderAllPages(pdf);
      });
    ...
    document.getElementById('zoomIn').addEventListener('click', function() {
      ...
      renderAllPages(pdfDoc); // Re-render with new scale
    });

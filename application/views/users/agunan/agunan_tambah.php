          ...
          <fieldset class="content-group">
            <div class="navigation-buttons">
              <div class="btn-group" style="float: left;">
                <i class="fa fa-id-card"></i> Tambah BPKB
              </div>
            ...
            <hr>
            <form class="form-horizontal" id="myForm" action="users/agunan_simpan" enctype="multipart/form-data" method="post">
              <div class="row">
...
<script>
  ...
  Dropzone.options.myDropzone = {
    url: "<?php echo base_url('users/agunan_simpan') ?>",
      ...
      this.on("success", function(file, response) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location = "<?php echo base_url(); ?>users/agunan";
          }
        });
      });
    }
  ...
  function submitFormWithoutFile() {
    ...
    var pembawa = $('#pembawa').val();
    var status = $('#status').val();
    var catatan_status = $('#catatan_status').val();
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('users/agunan_simpan'); ?>",
  ...
  $(document).ready(function() {
    $(".cari_anggota").select2({
      ajax: {
        url: "<?php echo base_url('get-agunan-data-bpkb'); ?>",
        dataType: "json",
        delay: 250,
        processResults: function(data) {
          return {
            results: $.map(data, function(item) {
              if (item.agunan1) {
              $('#agunan1-container').show();
              $('#agunan1').val(item.agunan1);
            } else {
              $('#agunan1-container').hide();
            }

            if (item.agunan2) {
              $('#agunan2-container').show();
              $('#agunan2').val(item.agunan2);
            } else {
              $('#agunan2-container').hide();
            }

            if (item.agunan3) {
              $('#agunan3-container').show();
              $('#agunan3').val(item.agunan3);
            } else {
              $('#agunan3-container').hide();
            }
              return {

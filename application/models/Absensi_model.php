...
class Absensi_model extends CI_Model
{
    public function get_absen($id_user, $bulan, $tahun)
    {
        ...
        $result = $this->db->get('tbl_absensi a');
    ...
    // {
    //     $result = $this->db->insert('tbl_absensi', $data);
    //     return $result;
    // }
    ...
    public function get_rekap_absensi($bulan, $tahun)
    {
        $query = $this->db->query("
            ...
            FROM tbl_user u
            LEFT JOIN tbl_absensi a ON u.id_user = a.id_user
    ...
    public function get_absen_all_users($id_user, $bulan, $tahun)
    {
        ...
        $this->db->where("DATE_FORMAT(tgl, '%m') = ", $bulan);
        $this->db->where("DATE_FORMAT(tgl, '%Y') = ", $tahun);
        $result = $this->db->get('tbl_absensi');
        return $result->result_array();
    }
    ...
    public function getDetailAbsenByDateRange($id_user, $tanggal_awal, $tanggal_akhir)
    {
        $this->db->select('DATE_FORMAT(tgl, "%d") as day, keterangan, waktu');
        $this->db->where('id_user', $id_user);
        $this->db->where('tgl >=', $tanggal_awal);
    ...
    public function getUserRekapDataByDateRange($id_user, $tanggal_awal, $tanggal_akhir)
    {
        $query = $this->db->query("
        SELECT 
            -- Hadir (lebih dari atau sama dengan 5 jam)
            SUM(CASE 
                WHEN a1.keterangan = 'Masuk' AND a2.keterangan = 'Pulang' 
                     AND TIME_TO_SEC(TIMEDIFF(a2.waktu, a1.waktu)) / 3600 >= 7 
                THEN 1 
                ELSE 0 
            END) AS hadir,
            
            -- Setengah hari (kurang dari 5 jam atau hanya masuk tanpa pulang)
            SUM(CASE 
                WHEN (a1.keterangan = 'Masuk' AND a2.keterangan = 'Pulang' 
                      AND TIME_TO_SEC(TIMEDIFF(a2.waktu, a1.waktu)) / 3600 < 7) 
                     OR (a1.keterangan = 'Masuk' AND a2.keterangan IS NULL) 
                THEN 1 
                ELSE 0 
            END) AS setengah_hari,
    
            -- Izin
            SUM(CASE 
                WHEN a1.keterangan = 'Izin' 
                THEN 1 
                ELSE 0 
            END) AS izin,
            ...
            -- Perjalanan tugas
            SUM(CASE 
                WHEN a1.keterangan = 'Perjalanan_tugas' 
                THEN 1 
                ELSE 0 
            END) AS perjalanan_tugas,
    
            -- Telat (masuk lebih dari jam 08:00)
            SUM(CASE 
                WHEN a1.keterangan = 'Masuk' AND TIME(a1.waktu) > '08:00:59' 
                THEN 1 
                ELSE 0 
            END) AS telat,
    
            -- Tidak masuk (hari kerja tanpa hadir atau alasan lainnya)
            (
                SELECT COUNT(*)
                FROM (
                    ...
                    CROSS JOIN (SELECT 0 AS b UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6) AS a2
                    CROSS JOIN (SELECT @row := 0) AS vars
                    WHERE DATE_ADD('{$tanggal_awal}', INTERVAL @row DAY) <= '{$tanggal_akhir}'
                ) dates
                LEFT JOIN tbl_absensi a1 ON a1.id_user = '{$id_user}' AND DATE(a1.tgl) = dates.date
                WHERE WEEKDAY(dates.date) < 5 -- Hanya hari kerja
                AND a1.id_user IS NULL
            ) AS tidak_masuk,
    
            -- Total hadir (hadir penuh + setengah hari)
            (
                SUM(CASE 
                    WHEN a1.keterangan = 'Masuk' AND a2.keterangan = 'Pulang' 
                         AND TIME_TO_SEC(TIMEDIFF(a2.waktu, a1.waktu)) / 3600 >= 5 
                    THEN 1 ELSE 0 END) + 
                SUM(CASE 
                    WHEN (a1.keterangan = 'Masuk' AND a2.keterangan = 'Pulang' 
                          AND TIME_TO_SEC(TIMEDIFF(a2.waktu, a1.waktu)) / 3600 < 5) 
        ...
        FROM tbl_absensi a1
        LEFT JOIN tbl_absensi a2 ON a1.id_user = a2.id_user AND DATE(a1.tgl) = DATE(a2.tgl) AND a2.keterangan = 'Pulang'
    ...
    {
        $this->db->select('tbl_user.id_user, tbl_user.nama_lengkap, tbl_user.kode_kantor, tbl_user.level, tbl_absensi.keterangan, tbl_absensi.keterangan_absen, tbl_absensi.waktu, tbl_absensi.lampiran, tbl_absensi.foto_absen, tbl_absensi.latitude, tbl_absensi.longitude, tbl_absensi.jns_absen');
        $this->db->from('tbl_user');
        $this->db->join('(SELECT id_user, MAX(waktu) AS max_waktu FROM tbl_absensi WHERE tgl = ' . $this->db->escape($tanggal) . ' GROUP BY id_user) latest_absensi', 'tbl_user.id_user = latest_absensi.id_user', 'left');
        $this->db->join('tbl_absensi', 'tbl_user.id_user = tbl_absensi.id_user AND tbl_absensi.waktu = latest_absensi.max_waktu', 'left');
        $this->db->where('(tbl_absensi.tgl = ' . $this->db->escape($tanggal) . ' OR tbl_absensi.tgl IS NULL)');
        $this->db->where(array(
            'tbl_user.id_user !=' => 0,
            'tbl_user.level !=' => 'satpam'
        ));
        $this->db->where('tbl_user.status !=', 'pending');
        // $this->db->order_by('tbl_absensi.waktu', 'ASC');
        $query = $this->db->get();
        return $query->result_array();
    }
}
